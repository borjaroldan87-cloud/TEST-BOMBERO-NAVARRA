...
1. FUENTE
- Determina si es una imagen técnica del temario, una referencia procedente
  de un examen oficial o una imagen no utilizable.
- No inventes contenido que no sea visible.
- Si existe pie de imagen, título, número de figura o texto explicativo,
  transcríbelo en sourceCaption/sourceText.
- Ese texto sirve para catalogar la imagen, pero NO debe formar parte de la
  imagen mostrada al opositor si revela información.

2. SEPARACIÓN
- Por defecto, cada dibujo, esquema, configuración o representación
  independiente debe convertirse en un asset independiente.
- Si una captura contiene 4 dibujos independientes, deben existir 4 assets.
- Si contiene 6 configuraciones independientes, deben existir 6 assets.
- No mantengas varios dibujos unidos únicamente porque aparecen juntos
  en la misma captura.
- Solo conserva una composición completa cuando la regla específica del tema
  permita expresamente utilizarla como conjunto.

3. CROP
- Para cada asset devuelve crop con coordenadas NORMALIZADAS entre 0 y 1.
- x e y indican la esquina superior izquierda.
- width y height indican el tamaño del recorte.
- El crop debe contener todo el dibujo necesario y excluir, siempre que sea
  posible, pies de imagen, títulos, párrafos y texto ajeno al propio esquema.
- No cortes símbolos, flechas, cotas, etiquetas técnicas necesarias ni partes
  esenciales del dibujo.

4. TEXTO
- textToRemove debe contener únicamente textos que NO deben mostrarse durante
  el examen porque identifican directamente la respuesta, explican el dibujo,
  incluyen el pie de figura o aportan información externa innecesaria.
- No elimines letras A/B/C/D, números, símbolos, cotas, magnitudes o etiquetas
  cuando sean parte funcional del dibujo y puedan ser necesarias para resolver
  una pregunta.

5. UTILIDAD PARA PREGUNTA GRÁFICA
- usableForGraphicQuestion=true únicamente cuando el dibujo permite formular
  una pregunta cuya resolución dependa realmente de observar la imagen.
- needsImageToAnswer=true únicamente cuando eliminar la imagen impediría
  responder correctamente a la pregunta prevista.
- Un dibujo meramente decorativo, redundante o que solo ilustra algo ya escrito
  debe rechazarse.
- No conviertas una imagen en pregunta gráfica si el enunciado tendría que
  describir aquello mismo que el opositor debería interpretar visualmente.

6. CONCEPTO
- concept debe identificar de forma breve y técnica qué representa el asset.
- visualDescription debe describir solo lo que aparece visualmente, sin
  desarrollar una pregunta ni anticipar una respuesta correcta.

7. AGRUPACIONES
- Respeta estrictamente las excepciones indicadas en REGLA DE AGRUPACIÓN.
- Fuera de esas excepciones, separa los dibujos siempre que sean
